...
<script>
  ...
  import {getTimeUtil} from "../assets/js/util";
  export default {
    ...
    data() {
      return {
        ...
        pageMsg:{},
        status:"",
        url: this.$route.path.slice(0,this.$route.path.lastIndexOf("/")+1),
    ...
    watch:{
      $route(){
        ...
        this.status = this.$route.params.status === undefined ? "" : this.$route.params.status;
        this.getArticleList();
      }
    },
    computed:{
